...
const generateAccessAndRefreshToken = async(userId)=>{
    try {
        ...
        // small check for user existence - TODO
...
const refreshAccessToken = asyncHandler(async(req,res)=>{
    ...
    try {
        ...
        if(!user){
            throw new ApiError(404,"Invalid Refresh Token") // we didn't found this Ref token in the mongoDB
        }

        if(incomingRefreshToken !== user?.refreshToken){
            throw new ApiError(401,"Invalid Refresh Token") // The incoming token didn't matched
        }
